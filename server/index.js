...
function canAccessReports(user) {
  if (canManagePermissions(user)) return true;
  return getUserPermission(user, "reports", user?.role === "host" ? "admin" : "none") !== "none";
}

function canEditReports(user) {
  if (canManagePermissions(user)) return true;
  return getUserPermission(user, "reports", user?.role === "host" ? "admin" : "none") === "admin";
}
...
function requireReportsAccess(request, response) {
  if (canAccessReports(request.user)) return true;
  response.status(403).json({ error: "Reports access required." });
  return false;
}

function requireReportsAdmin(request, response) {
  if (canEditReports(request.user)) return true;
  response.status(403).json({ error: "Reports admin access required." });
  return false;
}
  ...
  const rows = enumerateIsoDates(startIso, endIso).map((isoDate) => {
    ...
    return {
      ...
      cells: filteredVisibleStaff.map((staffEntry) => {
        ...
        let expectedClockInMinutes = contractedClockInMinutes;
        let expectedClockOutMinutes = contractedClockOutMinutes;
        const halfDayDuration = String(holidayEntry?.duration || "").trim().toLowerCase();
        if (halfDayDuration === "morning") {
          expectedClockInMinutes = ATTENDANCE_AFTERNOON_HOLIDAY_START_MINUTES;
        } else if (halfDayDuration === "afternoon") {
          expectedClockOutMinutes =
            contractedClockOutMinutes !== null
              ? Math.min(contractedClockOutMinutes, ATTENDANCE_MORNING_HOLIDAY_END_MINUTES)
              : ATTENDANCE_MORNING_HOLIDAY_END_MINUTES;
        }
...
function normalizeReportsCustomerName(value = "") {
  return String(value || "")
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/\b(limited|ltd|plc|llp|group)\b/g, " ")
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function getReportsSearchTerms(companyName = "") {
  const trimmed = String(companyName || "").trim();
  if (!trimmed) return [];
  const normalized = normalizeReportsCustomerName(trimmed);
  const variants = new Set([trimmed]);
...
function parseReportsDateValue(value) {
  const raw = String(value || "").trim();
  if (!raw) return "";

  const isoMatch = raw.match(/^(\d{4}-\d{2}-\d{2})/);
  if (isoMatch && isValidIsoDate(isoMatch[1])) {
    return isoMatch[1];
  }
...
function scoreReportsDateField(key = "", value = "") {
  const lowerKey = String(key || "").toLowerCase();
  const parsed = parseReportsDateValue(value);
  if (!parsed) return -1;

  let score = 0;
  if (/(invoice.*date|date.*invoice|invoicedt)/i.test(lowerKey)) score += 140;
  else if (/(order.*date|date.*order|transactiondate|documentdate)/i.test(lowerKey)) score += 120;
  else if (/(estimate.*date)/i.test(lowerKey)) score += 95;
  else if (/(createddt|createddate)/i.test(lowerKey)) score += 70;
  else if (/(modifieddt|modifieddate|updateddt|updateddate)/i.test(lowerKey)) score += 50;
  else if (/date/i.test(lowerKey)) score += 20;

  if (/(items?|lineitems?|components?)\./i.test(lowerKey)) score -= 60;
  if (/(due date|duedate|shipdate|deliverydate|requiredby|expiry)/i.test(lowerKey)) score -= 25;
  return score;
}

function resolveReportsOrderDate(order = {}) {
  const fields = Array.isArray(order.debugFields) ? order.debugFields : [];
  let best = { isoDate: "", sourceKey: "", score: -1 };

  fields.forEach((field) => {
    const score = scoreReportsDateField(field.key, field.value);
    if (score <= best.score) return;
    const isoDate = parseReportsDateValue(field.value);
    if (!isoDate) return;
    best = {
      isoDate,
      sourceKey: String(field.key || ""),
      score
    };
  });
...
function getReportsDocumentKind(order = {}) {
  const reference = String(order.orderReference || "").trim();
  const status = String(order.status || "").trim().toLowerCase();
  if (/^invs?-/i.test(reference) || /invoiced/.test(status)) return "Invoice";
  if (/^ests?-/i.test(reference)) return "Estimate";
  return "Order";
}

function doesReportsCustomerMatch(order = {}, companyName = "") {
  const target = normalizeReportsCustomerName(companyName);
  const candidate = normalizeReportsCustomerName(order.customerName || "");
  if (!target || !candidate) return false;
  if (candidate === target) return true;
  if (candidate.includes(target) || target.includes(candidate)) return true;
  const targetTokens = target.split(" ").filter(Boolean);
  return targetTokens.length
    ? targetTokens.every((token) => candidate.includes(token))
    : false;
}

function buildReportsDataset({ companyName = "", dateFrom = "", dateTo = "", orders = [] }) {
  const entries = orders
    .map((order) => {
      const dateInfo = resolveReportsOrderDate(order);
      const lineItems = extractProFormaLineItems(order).map((item) => ({
        name: String(item.name || "").trim(),
        description: String(item.description || "").trim(),
        quantity: Number(item.quantity || 0) || 0,
        unitPrice: roundProFormaMoney(Number(item.unitPrice || 0)),
        lineTotal: roundProFormaMoney(Number(item.lineTotal || 0))
      }));
      const subtotal = roundProFormaMoney(lineItems.reduce((sum, item) => sum + Number(item.lineTotal || 0), 0));
      const totals = extractProFormaTotals(order, subtotal, pickProFormaVatRate(order));
      return {
        id: String(order.id || ""),
        reference: String(order.orderReference || "").trim(),
        customerName: String(order.customerName || "").trim(),
        documentKind: getReportsDocumentKind(order),
        status: String(order.status || "").trim(),
        date: dateInfo.isoDate,
        dateSource: dateInfo.sourceKey,
        contact: String(order.contact || "").trim(),
        siteAddress: String(order.siteAddress || order.address || "").trim(),
        lineItems,
        subtotal,
        discountAmount: roundProFormaMoney(Number(totals.discountAmount || 0)),
        preTaxTotal: roundProFormaMoney(Number(totals.preTaxTotal || 0)),
        vatAmount: roundProFormaMoney(Number(totals.vatAmount || 0)),
        total: roundProFormaMoney(Number(totals.total || 0))
      };
    })
    .filter((entry) => doesReportsCustomerMatch({ customerName: entry.customerName }, companyName))
    .filter((entry) => entry.date && (!dateFrom || entry.date >= dateFrom) && (!dateTo || entry.date <= dateTo))
    .sort((left, right) => {
      if (left.date !== right.date) return String(left.date).localeCompare(String(right.date));
  ...
  return {
    ...
    lineItemCount: entries.reduce((sum, entry) => sum + entry.lineItems.length, 0),
    totalValue: roundProFormaMoney(entries.reduce((sum, entry) => sum + Number(entry.total || 0), 0)),
    entries
  };
}

function buildReportsFallbackText(dataset = {}, instructions = "") {
  const heading = `Report for ${dataset.companyName || "Selected customer"}`;
  const range = dataset.dateFrom && dataset.dateTo
    ? `Date range: ${dataset.dateFrom} to ${dataset.dateTo}`
    : "";
  const intro = [
    heading,
    range,
    `Documents found: ${dataset.orderCount || 0}`,
    `Line items found: ${dataset.lineItemCount || 0}`,
    `Combined total: ${formatProFormaMoney(dataset.totalValue || 0)}`
  ].filter(Boolean).join("\n");

  const body = (dataset.entries || []).map((entry) => {
    const lines = (entry.lineItems || []).map((line, index) =>
      `${index + 1}. ${line.name || "Line item"} | Qty ${line.quantity || 0} | Unit ${formatProFormaMoney(line.unitPrice || 0)} | Total ${formatProFormaMoney(line.lineTotal || 0)}${line.description ? `\n   ${line.description}` : ""}`
    );
    return [
      `${entry.documentKind}: ${entry.reference || "-"}`,
      `Date: ${entry.date || "-"}`,
      entry.status ? `Status: ${entry.status}` : "",
      `Customer: ${entry.customerName || "-"}`,
      `Order total: ${formatProFormaMoney(entry.total || 0)}`,
      ...lines
    ].filter(Boolean).join("\n");
  });
  ...
  return [
    intro,
    instructions ? `Requested focus: ${instructions}` : "",
    ...body
  ].filter(Boolean).join("\n\n").trim();
}

function getReportsAiStatus() {
  const apiKey = String(process.env.OPENAI_API_KEY || "").trim();
  return {
    configured: Boolean(apiKey),
    model: String(process.env.REPORTS_AI_MODEL || process.env.SOCIAL_POST_AI_MODEL || "gpt-4o-mini").trim(),
    keyLength: apiKey ? apiKey.length : 0
  };
}

async function generateReportsTextWithAi(dataset = {}, instructions = "") {
  const aiStatus = getReportsAiStatus();
  const fallback = buildReportsFallbackText(dataset, instructions);
  if (!aiStatus.configured) {
    return { report: fallback, source: "template", warning: "OpenAI API key not configured.", aiStatus };
  }

  const compactEntries = (dataset.entries || []).map((entry) => ({
    ...
    lineItems: (entry.lineItems || []).map((line) => ({
      name: line.name,
      description: String(line.description || "").slice(0, 220),
  ...
  const messages = [
    {
      role: "system",
      content: "You write accurate, professional business reports. Use only the supplied data. Do not invent missing orders, dates, prices, quantities or totals. Keep the report factual and client-facing. Use GBP formatting. Structure the report with a short overview followed by each order or invoice and its individual line items, quantities and prices."
    },
    {
      role: "user",
      content: JSON.stringify({
        task: instructions || `Create a report for ${dataset.companyName} listing all orders / invoices between ${dataset.dateFrom} and ${dataset.dateTo} and their individual line items, quantities and prices.`,
        companyName: dataset.companyName,
        dateFrom: dataset.dateFrom,
        dateTo: dataset.dateTo,
        orderCount: dataset.orderCount,
        lineItemCount: dataset.lineItemCount,
  ...
  try {
    ...
    const report = String(payload?.choices?.[0]?.message?.content || "").trim();
    if (!report) {
      throw new Error("OpenAI returned an empty report.");
    }
    return { report, source: "ai", warning: "", aiStatus };
  } catch (error) {
    return { report: fallback, source: "template", warning: error.message || "Could not generate the AI report.", aiStatus };
  }
}
...
function createServer() {
  ...
  app.post("/api/auth/login", async (request, response) => {
    ...
    const sessionUser = sanitizeUser(user);
        if (!canAccessBoard(sessionUser) && !canAccessInstaller(sessionUser) && !canAccessHolidays(sessionUser) && !canAccessAttendance(sessionUser) && !canAccessMileage(sessionUser) && !canAccessMaterials(sessionUser) && !canAccessVanEstimator(sessionUser) && !canAccessRams(sessionUser) && !canAccessSocialPost(sessionUser) && !canAccessDescriptionPull(sessionUser) && !canAccessProForma(sessionUser) && !canAccessReports(sessionUser)) {
...
app.get("/api/corebridge/orders", async (request, response) => {
  ...
  app.post("/api/reports/generate", async (request, response) => {
    if (!requireReportsAccess(request, response)) return;
    try {
      ...
      const instructions = String(request.body?.instructions || "").trim().slice(0, 4000);

      if (!companyName) {
        response.status(400).json({ error: "Enter a company name." });
        return;
      }
      if (!isValidIsoDate(dateFrom) || !isValidIsoDate(dateTo)) {
        response.status(400).json({ error: "Enter a valid start and end date." });
        return;
      }
      if (dateFrom > dateTo) {
        response.status(400).json({ error: "The end date must be on or after the start date." });
        return;
      }

      const resultsById = new Map();
      const lookupAttempts = [];
      for (const searchTerm of getReportsSearchTerms(companyName)) {
        try {
          const payload = await fetchCoreBridgeOrders(searchTerm, true, { includeClosed: true });
          lookupAttempts.push({
            searchTerm,
          ...
          (payload.orders || []).forEach((order) => {
            const key = String(order.id || order.orderReference || "").trim();
            if (!key) return;
            if (!resultsById.has(key)) {
              resultsById.set(key, order);
            }
          });
        } catch (lookupError) {
          lookupAttempts.push({
            searchTerm,
            count: 0,
            error: lookupError.message || "Lookup failed"
          });
        }
      }
      ...
      const dataset = buildReportsDataset({
        ...
        dateFrom,
        dateTo,
        orders: [...resultsById.values()]
      });

      const generated = await generateReportsTextWithAi(dataset, instructions);
      response.json({
        dataset,
      ...
      response.status(error.statusCode || 500).json({
        error: error.statusCode === 503 ? "CoreBridge is not configured yet." : "Could not generate the report.",
        detail: error.message
      });
    }
  });
