...
const DEFAULT_PRO_FORMA_TEMPLATE = {
  ...
  sections: {
    ...
    billing: { x: 13.2, y: 33.8, w: 36.3, h: 15.8 },
    company: { x: 136.4, y: 33.8, w: 60.2, h: 16.8 },
    metaLeft: { x: 13.5, y: 71.3, w: 57.1, h: 13.2 },
    metaRight: { x: 136.4, y: 71.3, w: 60.2, h: 7.2 },
    table: { x: 12, y: 96.6, w: 185.6, h: 68 },
    ...
    tableDescription: { x: 25.6, y: 116.8, w: 132.5, h: 13.2 },
    bank: { x: 14, y: 178.8, w: 58, h: 20.2 },
    totals: { x: 138.5, y: 180.9, w: 59.4, h: 27.2 },
    approval: { x: 13.3, y: 234.7, w: 184.3, h: 4.9 },
    paymentTerms: { x: 13.3, y: 246.1, w: 184.3, h: 8.3 },
    accreditations: { x: 12.8, y: 262.9, w: 184.4, h: 8.4 },
    footerMeta: { x: 12.8, y: 276.8, w: 184.4, h: 4.2 }
  }
};
...
function SocialPostPage({ currentUser, onLogout, notifications }) {
  ...
  async function deleteVoice(voice) {
    if (!voice || !canAdmin) return;
    const confirmed = window.confirm(`Delete "${voice.name}" from the tone of voice list?`);
    if (!confirmed) return;
    setDeletingVoiceId(voice.id);
    setError("");
    try {
      const response = await fetch(`/api/social-post/voices/${encodeURIComponent(voice.id)}`, {
        method: "DELETE"
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Could not delete tone of voice.");
      const nextVoices = Array.isArray(payload.voices) ? payload.voices : [];
      setVoices(nextVoices);
      setVoiceId((current) => {
        if (nextVoices.some((entry) => String(entry.id) === String(current))) return current;
        return nextVoices[0]?.id || "";
      });
      setToneViewerOpen(false);
      setSocialStatus((current) => current ? {
        ...current,
        voices: nextVoices.map((entry) => ({
          id: entry.id,
          name: entry.name,
      ...
      setError(deleteError.message || "Could not delete tone of voice.");
    } finally {
      setDeletingVoiceId("");
    }
  }
          ...
          {toneViewerOpen && selectedVoice ? (
            <div className="modal-backdrop social-post-tone-backdrop" onMouseDown={() => setToneViewerOpen(false)}>
              ...
              <div className="social-post-tone-layout">
                <section className="social-post-tone-traits">
                  ...
                  <textarea
                    ...
                    readOnly={!canAdmin}
                    onChange={(event) => setToneTraitsDraft(event.target.value)}
                    placeholder="Paste personality traits, rhythm, favourite phrases, humour rules, emoji habits and writing do/don't notes here."
                  />
                </section>
              ...
              <div className="social-post-tone-actions">
                ...
                {canAdmin ? (
                  <div className="social-post-tone-action-buttons">
                    <button className="text-button danger" type="button" onClick={() => deleteVoice(selectedVoice)} disabled={toneSaving || deletingVoiceId === selectedVoice.id}>
                      {deletingVoiceId === selectedVoice.id ? "Deleting..." : "Delete tone"}
                    </button>
                    <button className="primary-button" type="button" onClick={saveToneDraft} disabled={toneSaving || !toneDraft.trim()}>
                      {toneSaving ? <span className="button-spinner-label"><span className="button-spinner" />Saving...</span> : "Save tone"}
                    </button>
                  </div>
                ) : null}
              </div>
...
function RamsPage({ currentUser, onLogout, notifications, users = [] }) {
  ...
  async function saveCurrentRams() {
    ...
    try {
      ...
      const body = {
        ...
        cardOrder,
        edits: ramsEdits,
        viewPayload: buildCurrentRamsPdfPayload()
      };
  ...
  const getLiveProfileEdit = (key, fallback, liveValue) => {
    ...
    if (
      liveValue &&
      (!stored ||
        stored === "Phone number to be confirmed" ||
        stored === "RAMS prepared by" ||
        stored === "Director")
    ) {
      return liveValue;
    }
  ...
  const emergencyContacts = [
    {
      ...
      jobTitle: getLiveProfileEdit("emergencyContact-matt-title", "Director", mattProfile.jobTitle || ""),
      phone: getLiveProfileEdit("emergencyContact-matt-phone", "Phone number to be confirmed", mattProfile.phoneNumber || "")
    },
    {
      label: "Prepared by",
      name: getRamsEdit("emergencyContact-prepared-name", currentUser?.displayName || "Signs Express"),
      jobTitle: getLiveProfileEdit("emergencyContact-prepared-title", "RAMS prepared by", preparedByProfile.jobTitle || ""),
      phone: getLiveProfileEdit("emergencyContact-prepared-phone", "Phone number to be confirmed", preparedByProfile.phoneNumber || "")
    },
    {
      label: "Office",
            ...
            <div className="modal-backdrop rams-setup-backdrop" role="dialog" aria-modal="true">
              <div className="modal rams-setup-modal" ref={ramsSetupModalRef} tabIndex={-1}>
                ...
                <div className="modal-actions rams-setup-actions">
                  ...
                  <button className="primary-button" type="button" onClick={applyRamsSetup}>Apply to RAMS</button>
                </div>
              </div>
            </div>
          ) : null}
...
function ReadOnlyRamsDocument({
  ...
  const payload = document?.viewPayload && typeof document.viewPayload === "object" ? document.viewPayload : {};
      ...
      <div className="client-rams-amendment-box">
        <label>
          ...
          <textarea
            ...
            placeholder="Tell the RAMS creator what needs changing."
          />
        </label>
        <button
          className="ghost-button"
          type="button"
          onClick={() => onRequestAmendment(job, document, amendmentDraft)}
          disabled={amendmentSaving || !String(amendmentDraft || "").trim()}
        >
                    ...
                    <tr>
                      {staff.map((person) => (
                        <Fragment key={`summary-cols-${person.person}`}>
                          <th className="attendance-sub-head">In</th>
                          <th className="attendance-sub-head">Out</th>
                        </Fragment>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      ...
                      {staff.map((person) => {
                        const summaryEntry = attendanceSummary.find((entry) => entry.person === person.person);
                        return (
                          <Fragment key={`summary-overtime-${person.person}`}>
                            <td className="attendance-value-cell attendance-summary-value-cell attendance-summary-value-positive">
                    ...
                    <tr>
                      ...
                      {staff.map((person) => {
                        const summaryEntry = attendanceSummary.find((entry) => entry.person === person.person);
                        return (
                          <Fragment key={`summary-deductions-${person.person}`}>
                            <td className="attendance-value-cell attendance-summary-value-cell attendance-summary-value-negative">
...
const MATERIAL_REPORT_PROGRESS_STEPS = [
  "Pulling approved artwork from the morning meeting list",
  "Pulling job details from CoreBridge",
  "Walking line items and nested assemblies",
  "Simplifying the production details with AI",
  "Building the production report cards"
];

const MATERIAL_SHEET_PRESETS = [
  { value: "3050x1500", label: "3050 x 1500mm", width: 3050, height: 1500 },
  { value: "1220x2440", label: "1220 x 2440mm", width: 1220, height: 2440 },
...
function formatPlannerSize(width, height) {
  if (!(width > 0) || !(height > 0)) return "";
  return `${Math.round(width)} x ${Math.round(height)}mm`;
}

function getPlannerPresetSize(presetValue, presets, customWidth, customHeight) {
...
function formatPlannerQuantity(count) {
  const quantity = Math.max(0, Math.round(Number(count || 0)));
  return `${quantity}no.`;
}

function parsePlannerQuantityLabel(value) {
  const match = String(value || "").match(/(\d+(?:\.\d+)?)/);
  return match ? Number(match[1]) : 0;
}

function resolvePlannerQuantity(primaryValue, quantityLabel) {
  const numericPrimary = Number(primaryValue || 0);
  const labelQuantity = parsePlannerQuantityLabel(quantityLabel);
  return Math.max(
    numericPrimary > 0 ? numericPrimary : 0,
    labelQuantity > 0 ? labelQuantity : 0,
    1
  );
}
...
function formatPlannerHours(hours) {
  ...
  if (wholeHours && minutes) return `${wholeHours} hrs ${minutes} mins`;
  if (wholeHours) return `${wholeHours} hr${wholeHours === 1 ? "" : "s"}`;
  return `${minutes} mins`;
}
  ...
  return jobs.flatMap((job) => (Array.isArray(job.items) ? job.items : []).flatMap((item) => {
    ...
    return sourceNames.map((materialName, index) => {
      ...
      return {
        ...
        quantity: resolvePlannerQuantity(matched.quantity || item.quantity || 0, matched.quantityLabel || item.quantityLabel || ""),
...
function filterPanelPlannerRows(rows = []) {
  return rows.filter((row) => row.type === "sheet");
}

function groupPanelPlannerRows(rows = []) {
  ...
  rows.forEach((row) => {
    ...
    current.totalQuantity += Math.max(0, Math.round(resolvePlannerQuantity(row.quantity || 0, row.quantityLabel || "")));
    current.productionHours += Number(row.productionHours || 0) || 0;
...
function getSuggestedMaterialSheetSetting(group) {
  ...
  if (stockSizeHint) {
    if (stockSizeHint.height <= 10 || stockSizeHint.width <= 10) {
      const width = Math.max(stockSizeHint.width, stockSizeHint.height);
      return { preset: "custom", customWidth: Math.round(width), customHeight: "N/A", stockLabel: `${Math.round(width)}mm roll width` };
    }
...
function getMaterialSheetDimensions(setting = {}) {
  if (setting.preset === "custom") {
    ...
    const customHeight = String(setting.customHeight || "").trim().toLowerCase();
    const height = !customHeight || customHeight === "na" || customHeight === "n/a" ? 0 : Number(customHeight);
    return { width, height };
  }
...
function buildSheetPlanForGroup(rows, sheetWidth, sheetHeight) {
  ...
  const expandedPieces = rows.flatMap((row) => {
    const size = parsePlannerDimensions(row.finishedSize);
    const quantity = Math.max(0, Math.round(resolvePlannerQuantity(row.quantity || 0, row.quantityLabel || "")));
    if (!size || !quantity) return [];
    return Array.from({ length: quantity }, (_, index) => ({
      id: `${row.id}-${index + 1}`,
      width: size.width,
      height: size.height,
      label: formatPlannerSize(size.width, size.height),
      jobReference: row.jobReference
    }));
  }).sort((a, b) => (b.height * b.width) - (a.height * a.width));
  if (!expandedPieces.length || !(sheetWidth > 0)) return null;
  ...
  if (!(sheetHeight > 0)) {
    ...
    expandedPieces.forEach((piece) => {
      ...
      ].filter((entry, index, array) => index === 0 || entry.width !== array[0].width || entry.height !== array[0].height);

      let placed = false;
      for (const option of orientations) {
        if (option.width > sheetWidth) continue;
        const needsNewRow = cursorX > 0 && (cursorX + option.width > sheetWidth);
        const nextX = needsNewRow ? 0 : cursorX;
        const nextY = needsNewRow ? cursorY + rowHeight + kerf : cursorY;
        placements.push({
          ...piece,
          width: option.width,
        ...
        cursorX = nextX + option.width + kerf;
        cursorY = nextY;
        rowHeight = needsNewRow ? option.height : Math.max(rowHeight, option.height);
        placed = true;
        break;
      }
      if (!placed) {
        placements.push({
          ...
          height: piece.height,
          x: 0,
          y: cursorY + rowHeight + kerf
        });
        cursorX = piece.width + kerf;
        cursorY = cursorY + rowHeight + kerf;
        rowHeight = piece.height;
      }
    });

    const usedLength = placements.reduce((max, piece) => Math.max(max, piece.y + piece.height), 0);
    const usedArea = expandedPieces.reduce((total, piece) => total + (piece.width * piece.height), 0);
    return {
      totalSheets: 1,
      sheetWidth,
      sheetHeight: Math.max(usedLength, 1),
      sheetSizeLabel: `${Math.round(sheetWidth)}mm wide x ${Math.round(usedLength)}mm run`,
      totalWasteArea: Math.max(0, (sheetWidth * usedLength) - usedArea),
  ...
  const sheets = [];
  let currentSheet = { placements: [], cursorX: 0, cursorY: 0, rowHeight: 0 };

  const startNewSheet = () => {
    if (currentSheet.placements.length) {
      sheets.push({ placements: currentSheet.placements });
    }
    currentSheet = { placements: [], cursorX: 0, cursorY: 0, rowHeight: 0 };
  };
  ...
  expandedPieces.forEach((piece) => {
    ...
    const placeOnCurrentSheet = () => {
      for (const option of orientations) {
        if (option.width > sheetWidth || option.height > sheetHeight) continue;
        const needsNewRow = currentSheet.cursorX > 0 && (currentSheet.cursorX + option.width > sheetWidth);
        const nextX = needsNewRow ? 0 : currentSheet.cursorX;
        const nextY = needsNewRow ? currentSheet.cursorY + currentSheet.rowHeight + kerf : currentSheet.cursorY;
        if (nextY + option.height > sheetHeight) continue;
        ...
        currentSheet.cursorX = nextX + option.width + kerf;
        currentSheet.cursorY = nextY;
        currentSheet.rowHeight = needsNewRow ? option.height : Math.max(currentSheet.rowHeight, option.height);
        return true;
      }
      return false;
    };

    if (!placeOnCurrentSheet()) {
      startNewSheet();
      placeOnCurrentSheet();
    }
  });

  if (currentSheet.placements.length) sheets.push({ placements: currentSheet.placements });
...
function PanelSheetDiagram({ sheetPlan, materialName }) {
  if (!sheetPlan?.sheets?.length) return null;
  return (
    <div className="morning-meeting-sheet-diagrams">
      {sheetPlan.sheets.map((sheet, index) => (
        <figure key={`${materialName}-${index + 1}`} className="morning-meeting-sheet-figure">
          <svg
            className="morning-meeting-sheet-svg"
            viewBox={`0 0 ${sheetPlan.sheetWidth} ${sheetPlan.sheetHeight}`}
            role="img"
            aria-label={`${materialName} sheet ${index + 1}`}
          >
            <rect x="0" y="0" width={sheetPlan.sheetWidth} height={sheetPlan.sheetHeight} rx="18" ry="18" fill="#f8fafc" stroke="#94a3b8" strokeWidth="10" />
            {sheet.placements.map((piece, pieceIndex) => (
              <g key={piece.id}>
                <rect
                  ...
                  width={piece.width}
                  height={piece.height}
                  fill={pieceIndex % 2 === 0 ? "#dbeafe" : "#e9d5ff"}
                  stroke="#475569"
                  strokeWidth="6"
                  rx="12"
                  ry="12"
                ...
                <text
                  ...
                  fontSize={Math.max(42, Math.min(piece.width, piece.height) * 0.18)}
                  fill="#172033"
                  fontWeight="700"
                >
                  {piece.label}
          ...
          <figcaption>{sheetPlan.mode === "roll" ? `Run ${index + 1} - ${sheetPlan.sheetSizeLabel}` : `Sheet ${index + 1} of ${sheetPlan.totalSheets} - ${sheetPlan.sheetSizeLabel}`}</figcaption>
        </figure>
      ))}
    </div>
  );
...
function MorningMeetingMaterials({
  payload,
  loading,
  loadingStep,
  error,
  ...
  const panelGroups = groupPanelPlannerRows(panelRows);
  const plannerSummary = payload
    ? `${jobs.length} approved job${jobs.length === 1 ? "" : "s"} ready for production review`
    : "Build a clean production report from CoreBridge assemblies";
  return (
    <section className="morning-meeting-materials">
      <div className="morning-meeting-materials-head">
        <div className="morning-meeting-materials-summary">
          <span>Production requirements</span>
          <h2>Job Materials Report</h2>
          <div className="morning-meeting-material-status-row">
            <span className="morning-meeting-material-status-meta">{loading ? loadingStep : plannerSummary}</span>
            {payload ? <span className="morning-meeting-material-status-pill">{panelRows.length} panel line{panelRows.length === 1 ? "" : "s"} ready</span> : null}
            {loading ? <span className="morning-meeting-material-status-pill is-live"><span className="inline-spinner" aria-hidden="true" /> Working live</span> : null}
          </div>
        </div>
      ...
      {plannerOpen ? (
        <section className="morning-meeting-material-planner">
            ...
            {panelGroups.map((group) => (
              <article key={group.key} className="morning-meeting-material-order-group">
                  ...
                  {materialGroupSettings[group.key]?.preset === "custom" ? (
                    <>
                      ...
                      <label>
                        ...
                        <input
                          type="text"
                          list={`material-height-options-${group.key}`}
                          value={materialGroupSettings[group.key]?.customHeight || "N/A"}
                          onChange={(event) => onMaterialGroupSettingChange(group.key, "customHeight", event.target.value)}
                          placeholder="N/A"
                        />
                        <datalist id={`material-height-options-${group.key}`}>
                          <option value="N/A" />
                          <option value="1000" />
                          <option value="1220" />
                          <option value="1500" />
                ...
                <div className="morning-meeting-material-order-group-lines">
                  {group.rows.map((row) => (
                    <div key={row.id} className="morning-meeting-material-order-line">
                      <span>{row.jobReference}</span>
                      <strong>{row.quantityLabel}{row.finishedSize ? ` - ${row.finishedSize}` : ""}</strong>
                      <small>{row.lineItemTitle || row.customerName || "Panel item"}</small>
                    </div>
                  ))}
                </div>
              </article>
            ))}
            {!panelGroups.length ? <p className="morning-meeting-empty">No panel materials were returned from the approved jobs.</p> : null}
          </div>
          {finalisedPlan?.length ? (
            <div className="morning-meeting-material-plan-results">
              {finalisedPlan.map((group) => (
                <article key={group.key} className="morning-meeting-material-plan-card">
                  <div className="morning-meeting-material-plan-card-head">
                    <div>
                      <h4>{group.name}</h4>
                      <p>{formatPlannerQuantity(group.totalQuantity)} - {group.totalProductionLabel} estimated production time</p>
                    </div>
                    <span>{group.sheetPlan?.mode === "roll" ? "Run layout" : "Panel layout"}</span>
                  </div>
                  <div className="morning-meeting-material-plan-card-body">
                    {group.sheetPlan ? (
                      <div className="morning-meeting-material-plan-layout">
                      ...
                      <div className="morning-meeting-material-plan-block">
                        ...
                        <small>Set a numeric custom width, or use N/A height to work from width and run length.</small>
                      </div>
                    )}
                  </div>
                </article>
...
function MorningMeetingPage({ currentUser, onLogout, notifications }) {
  ...
  useEffect(() => {
    if (!materialsLoading) {
      setMaterialsLoadingStep(MATERIAL_REPORT_PROGRESS_STEPS[0]);
      return undefined;
    }
    let stepIndex = 0;
    setMaterialsLoadingStep(MATERIAL_REPORT_PROGRESS_STEPS[stepIndex]);
    const timer = window.setInterval(() => {
      stepIndex = Math.min(MATERIAL_REPORT_PROGRESS_STEPS.length - 1, stepIndex + 1);
      setMaterialsLoadingStep(MATERIAL_REPORT_PROGRESS_STEPS[stepIndex]);
    }, 1400);
    return () => window.clearInterval(timer);
  }, [materialsLoading]);
        ...
        {outline ? (
          <>
            ...
            <MorningMeetingMaterials
              ...
              loadingStep={materialsLoadingStep}
              error={materialsError}
              onFetch={fetchNeededMaterials}
              onPrint={printMaterials}
              plannerOpen={materialsPlannerOpen}
              onTogglePlanner={() => setMaterialsPlannerOpen((current) => !current)}
...
const IGLOO_STATUS_OPTIONS = [
  { value: "untested", label: "Template untested" },
  { value: "good", label: "Template good" },
  { value: "wrong", label: "Template wrong" }
];

function getIglooStatusLabel(status) {
  return IGLOO_STATUS_OPTIONS.find((entry) => entry.value === status)?.label || "Template untested";
}

function filterIglooCoolers(coolers, search, family) {
  const query = String(search || "").trim().toLowerCase();
  return (Array.isArray(coolers) ? coolers : []).filter((cooler) => {
    const matchesFamily = !family || cooler.family === family;
    const haystack = String((cooler.family || "") + " " + (cooler.model || "") + " " + (cooler.clientNote || "")).toLowerCase();
    return matchesFamily && (!query || haystack.includes(query));
  });
}

function IglooHeader({ signedOffCount, totalCount, admin = false, loading = false, onRefresh }) {
  return (
    <div className="igloo-brand-header">
      <div className="igloo-brand-lockup">
        <img className="igloo-logo" src="/branding/igloo-logo.svg" alt="IGLOO" />
        <span />
        <img className="igloo-sx-logo" src="/branding/signs-express-logo.svg" alt="Signs Express" />
      </div>
      <div className="igloo-header-actions">
        <span>{signedOffCount} of {totalCount} approved</span>
        {admin ? <a className="igloo-customer-view-button" href="/igloo" target="_blank" rel="noreferrer">Customer view</a> : null}
        {onRefresh ? <button type="button" onClick={onRefresh} disabled={loading}>{loading ? "Refreshing..." : "Refresh"}</button> : null}
      </div>
    </div>
  );
}

function IglooCatalogueToolbar({ coolers, search, family, onSearch, onFamily }) {
...
function IglooCatalogueRow({ cooler, admin = false, busy = false, noteBusy = false, onTemplateUpload, onPhotoUpload, onStatusChange, onCompletionUpload, onLinkEdit, onClientNoteSave }) {
  ...
  const isWrong = templateStatus === "wrong";
  ...
  const [clientNote, setClientNote] = useState(cooler.clientNote || "");

  useEffect(() => { setClientNote(cooler.clientNote || ""); }, [cooler.clientNote]);

  return (
    <article className={"igloo-catalogue-row" + (isGood ? " is-good" : isWrong ? " is-wrong" : "")}>
      <IglooPhotoUpload cooler={cooler} admin={admin} busy={busy} onPhotoUpload={onPhotoUpload} />
      <div className="igloo-product-cell">
        <span>{cooler.family}</span>
        <strong>{cooler.model}</strong>
        {!admin ? <div className="igloo-client-note"><input value={clientNote} onChange={(event) => setClientNote(event.target.value)} placeholder="Add a note" /><button type="button" onClick={() => onClientNoteSave?.(cooler, clientNote)} disabled={noteBusy || clientNote === (cooler.clientNote || "")}>{noteBusy ? "Saving" : "Save"}</button></div> : cooler.clientNote ? <small>{cooler.clientNote}</small> : null}
      </div>
      <div className="igloo-link-cell">
        {cooler.productUrl ? <a href={cooler.productUrl} target="_blank" rel="noreferrer" aria-label={'Open product page for ' + cooler.model}>??</a> : <span>-</span>}
        {admin ? <button type="button" onClick={() => onLinkEdit?.(cooler)} aria-label={'Edit product link for ' + cooler.model}>Edit</button> : null}
      </div>
      <div className="igloo-template-cell">
        {canUploadTemplate ? <label className="igloo-small-action is-upload-action">{busy ? "Uploading" : hasTemplate ? "Replace" : "Upload"}<input type="file" accept={IGLOO_TEMPLATE_ACCEPT} disabled={busy} onChange={(event) => { onTemplateUpload?.(cooler, event.target.files?.[0]); event.target.value = ""; }} /></label> : null}
        {hasTemplate ? <a className="igloo-small-action" href={cooler.templatePdfUrl} target="_blank" rel="noreferrer">View</a> : <span className="igloo-muted">-</span>}
      </div>
      <div className="igloo-approved-cell">
        {admin ? (
          <select className={"igloo-status-select " + templateStatus} value={templateStatus} onChange={(event) => onStatusChange?.(cooler, event.target.value)} disabled={busy}>
            {IGLOO_STATUS_OPTIONS.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}
          </select>
        ) : <span className={"igloo-approved-pill " + templateStatus}>{getIglooStatusLabel(templateStatus)}</span>}
      </div>
...
function IglooCustomerPage() {
  ...
  async function loadTemplateCatalogue() {
    ...
    try {
      ...
      setError(loadError.message || "Could not load IGLOO templates.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function uploadTemplate(cooler, file) {
    ...
    if (file.size > IGLOO_UPLOAD_LIMIT_BYTES) {
      setError("Template documents must be 10MB or under.");
      return;
    }
    setUploadingId(cooler.id);
    setError("");
    setMessage("");
    try {
      const dataUrl = await readFileAsDataUrl(file);
      const response = await fetch("/api/igloo/customer/coolers/" + encodeURIComponent(cooler.id) + "/template", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setData(payload);
      setMessage("Template uploaded for review.");
    } catch (uploadError) {
  ...
  async function saveClientNote(cooler, clientNote) {
    setNoteSavingId(cooler.id);
    setError("");
    setMessage("");
    try {
      const response = await fetch("/api/igloo/customer/coolers/" + encodeURIComponent(cooler.id) + "/note", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ clientNote })
      });
      const payload = await readJsonResponse(response, "Could not save note.");
      if (!response.ok) throw new Error(payload.error || "Could not save note.");
      setData(payload);
      setMessage("Note saved.");
    } catch (noteError) {
      setError(noteError.message || "Could not save note.");
    } finally {
      setNoteSavingId("");
    }
  }
  ...
  const visibleCoolers = filterIglooCoolers(coolers, search, family);
  const signedOffCount = coolers.filter((cooler) => (cooler.templateStatus || (cooler.templateTested ? "good" : "untested")) === "good").length;
...
function IglooAdminPage({ currentUser, onLogout, notifications }) {
  ...
  async function updateTemplateStatus(cooler, templateStatus) {
    ...
    setMessage("");
    try {
      const response = await fetch("/api/igloo/admin/coolers/" + encodeURIComponent(cooler.id) + "/template-status", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ templateStatus })
      });
      const payload = await readJsonResponse(response, "Could not update template status.");
      if (!response.ok) throw new Error(payload.error || "Could not update template status.");
      setData(payload);
      setMessage("Template status updated.");
    } catch (statusError) {
      setError(statusError.message || "Could not update template status.");
    } finally {
      setBusyId("");
    }
  }

  async function editProductLink(cooler) {
    const nextLink = window.prompt("Product link", cooler.productUrl || "");
    if (nextLink === null) return;
    setBusyId(cooler.id);
    setError("");
    setMessage("");
    try {
      const response = await fetch("/api/igloo/admin/coolers/" + encodeURIComponent(cooler.id) + "/product-link", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ productUrl: nextLink })
      });
      const payload = await readJsonResponse(response, "Could not update product link.");
      if (!response.ok) throw new Error(payload.error || "Could not update product link.");
      setData(payload);
      setMessage("Product link updated.");
    } catch (linkError) {
      setError(linkError.message || "Could not update product link.");
    } finally {
      setBusyId("");
    }
  }
  ...
  return (
    <div className="app-shell igloo-admin-shell"><div className="page"><MainNavBar currentUser={currentUser} active="igloo" onLogout={onLogout} notifications={notifications} />
      <section className="panel igloo-admin-panel">
        <IglooHeader admin signedOffCount={signedOffCount} totalCount={coolers.length} loading={loading} onRefresh={loadIglooAdmin} />
